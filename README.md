...
RHEL 9.8 인스턴스에서 실제로 실행해 확인한 것이다.

| 주장 | 확인 방법 |
|---|---|
| AMI 가 요청한 점 릴리스로 잡힌다 | `rhel_version = "9.8"` → `RHEL-9.8.0_HVM-20260728` |
| 첫 부팅이 실패 없이 끝난다 | 재생성 후 상태가 `complete`, 로그에 FAILED 0 건 |
| SSH 가 1 분 안에 열린다 | 패키지 설치가 끝나기 전에 접속됨 |
| 워치독이 패키지 설치보다 먼저 켜진다 | 부트 로그 순서상 `idle-watchdog` 이 첫 단계 |
| 워치독의 다음 실행이 실제로 잡힌다 | `NextElapseUSecMonotonic = 15min` |
| SELinux 가 Enforcing 이다 | 고객 RHEL 과 같은 기본값 |
| 재실행이 멱등이다 | `setup.sh` 두 번 실행 후 실패 0 건 |
| `vm` CLI 가 상태 없이 동작한다 | status / ssh / allow-ip 확인 |
